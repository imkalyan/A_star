...
int misplacedtiles(int initial[4][4], int final[4][4])
{
	int N=4;
	int i,j,p,q;
    int sum = 0;
...
NODE *extractmin(Queue *head)
{
	...
	while(temp!=NULL)
	{
		...
		if(min > temp->data.cost+temp->data.level){//printf("as1");																					//check
		//	printf("%d ",temp->data.cost);
...
_Bool check_solvability(int puzzle[4][4])
{
	int N=4;
    int invCount = Inv_count((int*)puzzle);
    if (N & 1)
        return !(invCount & 1);

    else 
    {
        int pos = find_pos(puzzle);
        if (pos & 1)
            return !(invCount & 1);
        else
            return invCount & 1;
